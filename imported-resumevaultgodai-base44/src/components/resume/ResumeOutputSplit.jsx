...
export default function ResumeOutputSplit({ title, content, icon: Icon, filename = 'resume' }) {
  const [copied, setCopied] = useState(false);

  // Extract only the resume — section 1 if structured, otherwise raw content
  const section1Match = content.match(/SECTION 1: ATS OPTIMIZED RESUME\n([\s\S]+?)(?=\n\nSECTION 2:|$)/i);
  const resumeContent = section1Match ? section1Match[1].trim() : content.trim();
  ...
  const handleWordDownload = () => {
    const htmlLines = resumeContent.split('\n').map(line => {
    ...
    <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
      ...
      <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100 bg-slate-50">
        ...
        <div className="flex gap-2">
          <Button variant="ghost" size="sm" onClick={handleCopy} className="text-slate-500 hover:text-slate-800 gap-1.5">
            {copied ? <Check className="w-4 h-4 text-green-600" /> : <Copy className="w-4 h-4" />}
            <span className="hidden sm:inline text-xs">{copied ? 'Copied!' : 'Copy'}</span>
          </Button>
          <Button variant="outline" size="sm" onClick={handlePDFDownload} className="text-slate-600 hover:text-red-600 hover:border-red-300 gap-1.5">
            <FileDown className="w-4 h-4" />
            <span className="hidden sm:inline text-xs">PDF</span>
          </Button>
          <Button variant="outline" size="sm" onClick={handleWordDownload} className="text-slate-600 hover:text-blue-600 hover:border-blue-300 gap-1.5">
      ...
      {/* Resume Content — full width, clean */}
      <div className="p-8 prose prose-sm prose-slate max-w-none max-h-[75vh] overflow-y-auto">
        <ReactMarkdown
          components={{
            h1: ({ children }) => <h1 className="text-2xl font-bold text-slate-900 mt-4 mb-2 text-center">{children}</h1>,
            h2: ({ children }) => <h2 className="text-base font-bold text-slate-800 mt-5 mb-2 border-b-2 border-slate-300 pb-1 uppercase tracking-wide">{children}</h2>,
            h3: ({ children }) => <h3 className="text-sm font-semibold text-slate-700 mt-3 mb-1">{children}</h3>,
            p: ({ children }) => <p className="text-slate-700 leading-relaxed my-1.5 text-sm">{children}</p>,
