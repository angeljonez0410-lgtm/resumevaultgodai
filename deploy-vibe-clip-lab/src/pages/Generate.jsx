...
import { getLocalCharacters } from '@/lib/local-characters';
...
export default function Generate() {
  ...
  const { data: charactersResponse = [] } = useQuery({
    ...
    queryFn: async () => {
      try {
        const remoteCharacters = asArray(await base44.entities.Character.list());
  ...
  const handleImageGenerated = async (result) => {
    ...
    if (selectedCharacter) {
      ...
      try {
        ...
        // Generation is still useful even if the optional project save fails.
      }
    }
  };
  ...
  const handleVideoCaptioned = (videoId, captionedUrl) => {
    setGeneratedVideos(prev => prev.map(video => (
      video.id === videoId
        ? { ...video, url: captionedUrl, captioned: true }
        : video
    )));
  };
  ...
  return (
    <div className="p-6 lg:p-10 max-w-6xl mx-auto">
      ...
      <div className="mb-6 max-w-xs">
        <label className="text-sm font-medium text-slate-300 mb-2 block">Character Context Optional</label>
        <Select value={selectedCharacter} onValueChange={setSelectedCharacter}>
          <SelectTrigger className="bg-slate-900 border-white/10 text-white">
            <SelectValue placeholder="Choose a character or generate from prompt..." />
          </SelectTrigger>
        ...
        <p className="text-xs text-slate-500 mt-2">You can generate realistic images and videos from a prompt even without a saved character.</p>
      </div>
              ...
              <div className="mt-8">
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                  {generatedImages.map((img, idx) => (
                    ...
                    >
                      <img src={img.url} alt="" className="w-full aspect-[3/2] object-cover group-hover:scale-105 transition-transform duration-300" />
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                  {generatedVideos.map((vid, idx) => (
                    ...
                    >
                      <VideoPlayer
                        videoUrl={vid.url}
                        projectTitle={vid.captioned ? `${selectedCharacterObj?.name || 'video'}-captioned` : selectedCharacterObj?.name}
                        onCaptioned={(captionedUrl) => handleVideoCaptioned(vid.id, captionedUrl)}
                      />
                    </motion.div>
                  ))}
                </div>
