      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-bold text-slate-800 flex items-center gap-2">
            <Users className="w-6 h-6 text-[#f4c542]" /> Users
          </h1>
          <p className="text-sm text-slate-500 mt-1">
            Manage users, credits, and roles
          </p>
        </div>
        <button
          onClick={fetchUsers}
                ...
                {filtered.map((user) => (
                  <tr key={user.user_id} className="border-b border-slate-100 hover:bg-slate-50">
                    <td className="px-4 py-3">
                      <p className="font-medium text-slate-800">{user.full_name || "ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â"}</p>
                    ...
                    <td className="px-4 py-3">
                      ...
                      >
                        {user.role === "admin" ? "Owner" : user.role || "user"}
                      </span>
                    </td>
                    <td className="px-4 py-3">
